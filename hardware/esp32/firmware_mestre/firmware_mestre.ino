...
const char* ID_DISPOSITIVO = "ESQ"; // ID ÚNICO para o computador
WiFiUDP udp;
...
void setup() {
    ...
    if (!mpuQuadril.begin(0x68, &Wire)) {
        Serial.println("MESTRE: Falha MPU Quadril. Verifique a conexao!");
    } else {
        Serial.println("MESTRE: MPU Quadril OK.");
    }
    
    // Inicializa MPU 2 (Coxa)
    if (!mpuCoxa.begin(0x68, &Wire1)) {
        Serial.println("MESTRE: Falha MPU Coxa.");
    } else {
        Serial.println("MESTRE: MPU Coxa OK.");
    }
