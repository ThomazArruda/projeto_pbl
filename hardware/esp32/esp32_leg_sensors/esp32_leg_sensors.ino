 ...
 * 4. Aplica um Filtro Complementar em cada IMU para obter um ângulo (Pitch).
 * 5. Calcula o Ângulo Relativo (Angulo1 - Angulo2).
 * 6. Envia apenas 3 valores: E:valor, C:valor, A:valor_float
 *
 * Correção V7: Aplica 'fabs()' (módulo/valor absoluto) ao ângulo relativo,
 * garantindo que o valor enviado seja sempre positivo.
 */
...
// A constante 'RAD_TO_DEG' já é definida por Arduino.h
...
void setup() {
  ...
  Serial.println("--- Firmware V7 (Ângulo Absoluto) ---");
...
void loop() {
  ...
  if (current_time - last_serial_time >= SERIAL_PERIOD) {
    ...
    // <<< CORREÇÃO AQUI: Aplicado fabs() para pegar o módulo (valor absoluto)
    float angulo_relativo = fabs(pitch1 - pitch2); 
